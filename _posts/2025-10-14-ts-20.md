---
title: "[TS] HTTP와 API 다루기 - 5"
date: 2025-10-14 23:30:00 +0900
categories: [웹 개발, 타입스크립트]
tags: [웹개발, 타입스크립트, TS, http, api]
pin: true
math: true
mermaid: true
---

> **학습 목표**
> - HTTP 프로토콜이 무엇인지 이해한다
> - REST API의 개념과 설계 원칙을 안다
> - TypeScript에서 API를 호출하고 응답을 처리할 수 있다
> - async/await를 사용하여 비동기 코드를 작성할 수 있다
> - 타입 안전한 API 통신을 구현할 수 있다
> - 에러 처리와 로딩 상태를 관리할 수 있다

--- 

### 과제 3: 재사용 가능한 API 서비스 클래스

제네릭과 에러 처리를 포함한 범용 API 서비스 클래스를 만들어보세요.

**요구사항:**
- 제네릭을 활용한 타입 안전성
- 모든 HTTP 메서드 지원
- 에러 처리 및 재시도 로직
- 로딩 상태 관리
- 요청/응답 인터셉터

```typescript
// 여기를 완성하세요
```

#### 정답 3: 재사용 가능한 API 서비스 클래스

```typescript
// API 응답 래퍼
interface ApiResponse<T> {
  success: boolean;
  data: T | null;
  error: string | null;
  statusCode: number;
}

// 요청 옵션
interface RequestOptions extends RequestInit {
  retry?: number;        // 재시도 횟수
  retryDelay?: number;   // 재시도 간격 (ms)
  timeout?: number;      // 타임아웃 (ms)
}

// 인터셉터 타입
type RequestInterceptor = (config: RequestInit) => RequestInit | Promise<RequestInit>;
type ResponseInterceptor = (response: Response) => Response | Promise<Response>;
type ErrorInterceptor = (error: Error) => Error | Promise<Error>;

class ApiService {
  private baseUrl: string;
  private defaultHeaders: Record<string, string>;
  private requestInterceptors: RequestInterceptor[] = [];
  private responseInterceptors: ResponseInterceptor[] = [];
  private errorInterceptors: ErrorInterceptor[] = [];
  private loadingStates: Map<string, boolean> = new Map();
  
  constructor(baseUrl: string, defaultHeaders: Record<string, string> = {}) {
    this.baseUrl = baseUrl;
    this.defaultHeaders = {
      "Content-Type": "application/json",
      ...defaultHeaders
    };
  }
  
  // 헤더 설정
  setHeader(key: string, value: string): void {
    this.defaultHeaders[key] = value;
  }
  
  // 헤더 삭제
  removeHeader(key: string): void {
    delete this.defaultHeaders[key];
  }
  
  // 인증 토큰 설정
  setAuthToken(token: string): void {
    this.setHeader("Authorization", \`Bearer \${token}\`);
  }
  
  // 인터셉터 추가
  addRequestInterceptor(interceptor: RequestInterceptor): void {
    this.requestInterceptors.push(interceptor);
  }
  
  addResponseInterceptor(interceptor: ResponseInterceptor): void {
    this.responseInterceptors.push(interceptor);
  }
  
  addErrorInterceptor(interceptor: ErrorInterceptor): void {
    this.errorInterceptors.push(interceptor);
  }
  
  // 로딩 상태
  isLoading(key: string): boolean {
    return this.loadingStates.get(key) || false;
  }
  
  private setLoading(key: string, loading: boolean): void {
    this.loadingStates.set(key, loading);
  }
  
  // 타임아웃 처리
  private async fetchWithTimeout(
    url: string,
    options: RequestInit,
    timeout: number
  ): Promise<Response> {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), timeout);
    
    try {
      const response = await fetch(url, {
        ...options,
        signal: controller.signal
      });
      clearTimeout(timeoutId);
      return response;
    } catch (error) {
      clearTimeout(timeoutId);
      if (error instanceof Error && error.name === "AbortError") {
        throw new Error("요청 시간 초과");
      }
      throw error;
    }
  }
  
  // 재시도 로직
  private async fetchWithRetry(
    url: string,
    options: RequestInit,
    retry: number,
    retryDelay: number,
    timeout: number
  ): Promise<Response> {
    let lastError: Error | null = null;
    
    for (let i = 0; i <= retry; i++) {
      try {
        const response = await this.fetchWithTimeout(url, options, timeout);
        return response;
      } catch (error) {
        lastError = error instanceof Error ? error : new Error("알 수 없는 에러");
        
        if (i < retry) {
          console.log(\`재시도 \${i + 1}/\${retry}...\`);
          await new Promise(resolve => setTimeout(resolve, retryDelay));
        }
      }
    }
    
    throw lastError;
  }
  
  // 핵심 요청 메서드
  private async request<T>(
    endpoint: string,
    options: RequestOptions = {}
  ): Promise<ApiResponse<T>> {
    const {
      retry = 0,
      retryDelay = 1000,
      timeout = 10000,
      ...fetchOptions
    } = options;
    
    const url = \`\${this.baseUrl}\${endpoint}\`;
    const loadingKey = \`\${fetchOptions.method || "GET"}:\${endpoint}\`;
    
    this.setLoading(loadingKey, true);
    
    try {
      // 요청 설정
      let config: RequestInit = {
        ...fetchOptions,
        headers: {
          ...this.defaultHeaders,
          ...fetchOptions.headers
        }
      };
      
      // 요청 인터셉터 실행
      for (const interceptor of this.requestInterceptors) {
        config = await interceptor(config);
      }
      
      // API 호출 (재시도 포함)
      let response = await this.fetchWithRetry(
        url,
        config,
        retry,
        retryDelay,
        timeout
      );
      
      // 응답 인터셉터 실행
      for (const interceptor of this.responseInterceptors) {
        response = await interceptor(response);
      }
      
      // HTTP 에러 체크
      if (!response.ok) {
        throw new Error(\`HTTP error! status: \${response.status}\`);
      }
      
      // JSON 파싱
      const data: T = await response.json();
      
      return {
        success: true,
        data,
        error: null,
        statusCode: response.status
      };
    } catch (error) {
      let processedError = error instanceof Error 
        ? error 
        : new Error("알 수 없는 에러");
      
      // 에러 인터셉터 실행
      for (const interceptor of this.errorInterceptors) {
        processedError = await interceptor(processedError);
      }
      
      console.error(\`API 에러 [\${loadingKey}]:\`, processedError.message);
      
      return {
        success: false,
        data: null,
        error: processedError.message,
        statusCode: 0
      };
    } finally {
      this.setLoading(loadingKey, false);
    }
  }
  
  // GET 요청
  async get<T>(endpoint: string, options: RequestOptions = {}): Promise<ApiResponse<T>> {
    return this.request<T>(endpoint, { ...options, method: "GET" });
  }
  
  // POST 요청
  async post<T>(
    endpoint: string,
    data?: any,
    options: RequestOptions = {}
  ): Promise<ApiResponse<T>> {
    return this.request<T>(endpoint, {
      ...options,
      method: "POST",
      body: data ? JSON.stringify(data) : undefined
    });
  }
  
  // PUT 요청
  async put<T>(
    endpoint: string,
    data?: any,
    options: RequestOptions = {}
  ): Promise<ApiResponse<T>> {
    return this.request<T>(endpoint, {
      ...options,
      method: "PUT",
      body: data ? JSON.stringify(data) : undefined
    });
  }
  
  // PATCH 요청
  async patch<T>(
    endpoint: string,
    data?: any,
    options: RequestOptions = {}
  ): Promise<ApiResponse<T>> {
    return this.request<T>(endpoint, {
      ...options,
      method: "PATCH",
      body: data ? JSON.stringify(data) : undefined
    });
  }
  
  // DELETE 요청
  async delete<T>(
    endpoint: string,
    options: RequestOptions = {}
  ): Promise<ApiResponse<T>> {
    return this.request<T>(endpoint, { ...options, method: "DELETE" });
  }
}

// 사용 예시
interface User {
  id: number;
  name: string;
  email: string;
}

interface Post {
  id: number;
  title: string;
  body: string;
  userId: number;
}

async function testApiService() {
  // API 서비스 생성
  const api = new ApiService("https://jsonplaceholder.typicode.com");
  
  // 요청 인터셉터 - 모든 요청 전에 로그
  api.addRequestInterceptor((config) => {
    console.log("📤 요청:", config.method, "시간:", new Date().toISOString());
    return config;
  });
  
  // 응답 인터셉터 - 모든 응답 후에 로그
  api.addResponseInterceptor((response) => {
    console.log("📥 응답:", response.status, response.statusText);
    return response;
  });
  
  // 에러 인터셉터 - 에러 발생 시 로그
  api.addErrorInterceptor((error) => {
    console.error("❌ 에러 인터셉터:", error.message);
    return error;
  });
  
  console.log("=== API Service 종합 테스트 ===\n");
  
  // 1. GET 요청
  console.log("1. GET 요청 - 사용자 목록");
  const usersResponse = await api.get<User[]>("/users");
  
  if (usersResponse.success && usersResponse.data) {
    console.log(\`✅ \${usersResponse.data.length}명의 사용자 조회 완료\`);
    usersResponse.data.slice(0, 3).forEach(user => {
      console.log(\`  - \${user.name} (\${user.email})\`);
    });
  }
  
  // 2. GET 요청 (단일 리소스)
  console.log("\n2. GET 요청 - 특정 사용자");
  const userResponse = await api.get<User>("/users/1");
  
  if (userResponse.success && userResponse.data) {
    console.log(\`✅ 사용자: \${userResponse.data.name}\`);
  }
  
  // 3. POST 요청
  console.log("\n3. POST 요청 - 게시글 생성");
  const newPost = {
    title: "TypeScript API Service",
    body: "재사용 가능한 API 서비스를 만들었습니다!",
    userId: 1
  };
  
  const createResponse = await api.post<Post>("/posts", newPost);
  
  if (createResponse.success && createResponse.data) {
    console.log(\`✅ 게시글 생성 완료 (ID: \${createResponse.data.id})\`);
    console.log(\`  제목: \${createResponse.data.title}\`);
  }
  
  // 4. PATCH 요청
  console.log("\n4. PATCH 요청 - 게시글 수정");
  const updateResponse = await api.patch<Post>("/posts/1", {
    title: "수정된 제목"
  });
  
  if (updateResponse.success && updateResponse.data) {
    console.log(\`✅ 게시글 수정 완료\`);
    console.log(\`  새 제목: \${updateResponse.data.title}\`);
  }
  
  // 5. DELETE 요청
  console.log("\n5. DELETE 요청 - 게시글 삭제");
  const deleteResponse = await api.delete("/posts/1");
  
  if (deleteResponse.success) {
    console.log("✅ 게시글 삭제 완료");
  }
  
  // 6. 에러 처리 테스트 (존재하지 않는 리소스)
  console.log("\n6. 에러 처리 테스트");
  const errorResponse = await api.get<User>("/users/999999");
  
  if (!errorResponse.success) {
    console.log(\`❌ 예상된 에러: \${errorResponse.error}\`);
  }
  
  // 7. 재시도 테스트 (잘못된 URL)
  console.log("\n7. 재시도 테스트");
  const retryApi = new ApiService("https://invalid-url-12345.com");
  const retryResponse = await retryApi.get<any>("/test", {
    retry: 2,
    retryDelay: 500,
    timeout: 2000
  });
  
  if (!retryResponse.success) {
    console.log(\`❌ 재시도 후 실패: \${retryResponse.error}\`);
  }
  
  // 8. 로딩 상태 테스트
  console.log("\n8. 로딩 상태 테스트");
  const loadingTest = api.get<User[]>("/users");
  console.log("로딩 중:", api.isLoading("GET:/users")); // true
  await loadingTest;
  console.log("로딩 완료:", api.isLoading("GET:/users")); // false
}

// 실행
testApiService();
```

---
