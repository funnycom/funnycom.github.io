...
┌────────────────────────────┐
│  function add(a, b) {      │
│    const sum = a + b;      │  ← 10 + 20 = 30
│    return sum;             │  ← 30을 반환하고 함수 종료
│  }                         │
└────────────────────────────┘
↓
result에 30이 저장됨
...
다음 시간에는 **조건문과 반복문**을 배웁니다!
- if/else로 조건에 따라 다른 동작 실행하기
- for문으로 반복 작업 자동화하기
- 구구단 출력기 만들기
- AI와 함께 복잡한 로직 만들어보기

**미리 생각해보기:**
지금까지는 코드가 위에서 아래로 한 번씩만 실행되었습니다. 만약 조건에 따라 다르게 동작하거나, 같은 작업을 여러 번 반복할 수 있다면? 프로그램이 훨씬 더 똑똑해질 거예요! 
