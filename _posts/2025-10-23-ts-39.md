---
title: "[TS] 고급 타입 기법 - 2"
date: 2025-10-23 14:05:00 +0900
categories: [웹 개발, 타입스크립트]
tags: [웹개발, 타입스크립트, TS, 고급타입]
pin: true
math: true
mermaid: true
published: false
---

## 🎯 학습 목표

* 조건부 타입으로 타입을 동적으로 생성할 수 있다
* infer 키워드로 타입을 추론하고 추출할 수 있다
* 매핑 타입으로 기존 타입을 변환할 수 있다
* 템플릿 리터럴 타입으로 문자열 타입을 조작할 수 있다
* 사용자 정의 타입 가드를 만들 수 있다
* is와 asserts 키워드를 활용할 수 있다
* 타입 단언을 올바르게 사용할 수 있다
* 인덱스 접근 타입으로 중첩된 타입을 추출할 수 있다

---

## 14.3 템플릿 리터럴 타입

템플릿 리터럴 타입은 **문자열 타입을 조작하고 결합**할 수 있는 강력한 기능입니다.

### 기본 문법
```typescript
// 문자열 리터럴 결합
type World = "world";
type Greeting = `hello ${World}`;  // "hello world"

// 유니온 타입과 결합
type Color = "red" | "blue" | "green";
type HexColor = `#${Color}`;
// "#red" | "#blue" | "#green"

// 여러 유니온 타입 조합
type Size = "small" | "large";
type Style = "primary" | "secondary";
type ButtonClass = `btn-${Size}-${Style}`;
// "btn-small-primary" | "btn-small-secondary" | 
// "btn-large-primary" | "btn-large-secondary"
```

### 문자열 조작

TypeScript는 4가지 내장 문자열 조작 타입을 제공합니다:
```typescript
// 1. Uppercase - 대문자로
type Upper = Uppercase<"hello">;  // "HELLO"

// 2. Lowercase - 소문자로
type Lower = Lowercase<"HELLO">;  // "hello"

// 3. Capitalize - 첫 글자만 대문자
type Cap = Capitalize<"hello">;   // "Hello"

// 4. Uncapitalize - 첫 글자만 소문자
type Uncap = Uncapitalize<"Hello">; // "hello"
```

**실전 예제 1: HTTP 메서드 타입**
```typescript
type Method = "get" | "post" | "put" | "delete";
type UpperMethod = Uppercase<Method>;
// "GET" | "POST" | "PUT" | "DELETE"

// HTTP 헤더
type HttpHeader = `X-${Capitalize<string>}`;

const header1: HttpHeader = "X-Api-Key";     // ✅ OK
const header2: HttpHeader = "X-Auth-Token";  // ✅ OK
const header3: HttpHeader = "x-custom";      // ❌ 에러! 'X-' 뒤는 대문자로 시작
```

**실전 예제 2: 이벤트 핸들러**
```typescript
type EventName = "click" | "focus" | "blur";
type EventHandler = `on${Capitalize<EventName>}`;
// "onClick" | "onFocus" | "onBlur"

interface ButtonProps {
  onClick?: () => void;
  onFocus?: () => void;
  onBlur?: () => void;
}

// 동적으로 핸들러 타입 생성
type HandlerProps<T extends string> = {
  [K in T as `on${Capitalize<K>}`]?: () => void;
};

type ButtonHandlers = HandlerProps<EventName>;
/*
{
  onClick?: () => void;
  onFocus?: () => void;
  onBlur?: () => void;
}
*/
```

### 패턴 매칭
```typescript
// 문자열 패턴 추출
type ExtractId<T extends string> = 
  T extends `${infer _Start}/${infer Id}/${infer _End}`
    ? Id
    : never;

type UserId = ExtractId<"/users/123/profile">;  // "123"
type PostId = ExtractId<"/posts/456/comments">; // "456"

// 실전 사용: URL 파싱
type ParseURL<T extends string> = 
  T extends `${infer Protocol}://${infer Domain}/${infer Path}`
    ? {
        protocol: Protocol;
        domain: Domain;
        path: Path;
      }
    : never;

type Parsed = ParseURL<"https://example.com/api/users">;
/*
{
  protocol: "https";
  domain: "example.com";
  path: "api/users";
}
*/
```

**실전 예제 3: 경로 파라미터 추출**
```typescript
// "/users/:id/posts/:postId" → { id: string; postId: string }
type ExtractParams<Path extends string> = 
  Path extends `${infer _Start}:${infer Param}/${infer Rest}`
    ? { [K in Param | keyof ExtractParams<Rest>]: string }
    : Path extends `${infer _Start}:${infer Param}`
      ? { [K in Param]: string }
      : {};

type UserPostParams = ExtractParams<"/users/:id/posts/:postId">;
/*
{
  id: string;
  postId: string;
}
*/

// 사용
function getRoute<Path extends string>(
  path: Path,
  params: ExtractParams<Path>
) {
  let result = path as string;
  for (const [key, value] of Object.entries(params)) {
    result = result.replace(`:${key}`, value);
  }
  return result;
}

const url = getRoute("/users/:id/posts/:postId", {
  id: "123",
  postId: "456"
});
// "/users/123/posts/456"
```

### SQL 쿼리 타입 안전성
```typescript
// SQL 컬럼 추출
type ExtractColumns<Q extends string> = 
  Q extends `SELECT ${infer Cols} FROM ${infer _Rest}`
    ? Cols extends `${infer Col}, ${infer Rest}`
      ? Col | ExtractColumns<`SELECT ${Rest} FROM _`>
      : Cols
    : never;

type Columns = ExtractColumns<"SELECT id, name, email FROM users">;
// "id" | "name" | "email"

// WHERE 조건 추출
type ExtractWhere<Q extends string> = 
  Q extends `${infer _Start} WHERE ${infer Condition}`
    ? Condition
    : never;

type WhereClause = ExtractWhere<"SELECT * FROM users WHERE age > 18">;
// "age > 18"
```

### 타입 레벨 문자열 연산

**예제 1: 문자열 분할**
```typescript
type Split<S extends string, D extends string> = 
  S extends `${infer Head}${D}${infer Tail}`
    ? [Head, ...Split<Tail, D>]
    : [S];

type Path = Split<"api/users/123", "/">;
// ["api", "users", "123"]

type Email = Split<"user@example.com", "@">;
// ["user", "example.com"]
```

**예제 2: 문자열 결합**
```typescript
type Join<T extends string[], D extends string> = 
  T extends [infer First extends string, ...infer Rest extends string[]]
    ? Rest extends []
      ? First
      : `${First}${D}${Join<Rest, D>}`
    : "";

type Path2 = Join<["api", "users", "123"], "/">;
// "api/users/123"

type Email2 = Join<["user", "example.com"], "@">;
// "user@example.com"
```

**예제 3: 문자열 길이**
```typescript
type StringLength
  S extends string,
  Acc extends any[] = []
> = S extends `${infer _First}${infer Rest}`
  ? StringLength<Rest, [...Acc, any]>
  : Acc['length'];

type Len1 = StringLength<"hello">;  // 5
type Len2 = StringLength<"TypeScript">;  // 10
```

### CSS 클래스 타입 안전성
```typescript
type Size = "sm" | "md" | "lg";
type Variant = "primary" | "secondary" | "danger";
type State = "hover" | "active" | "disabled";

// 버튼 클래스 생성
type ButtonClass = 
  | `btn-${Size}`
  | `btn-${Variant}`
  | `btn-${State}`;

/*
"btn-sm" | "btn-md" | "btn-lg" |
"btn-primary" | "btn-secondary" | "btn-danger" |
"btn-hover" | "btn-active" | "btn-disabled"
*/

// 타입 안전한 클래스 빌더
function buildButtonClass(
  size: Size,
  variant: Variant,
  state?: State
): string {
  const classes: ButtonClass[] = [`btn-${size}`, `btn-${variant}`];
  if (state) {
    classes.push(`btn-${state}`);
  }
  return classes.join(" ");
}

const className = buildButtonClass("md", "primary", "hover");
// "btn-md btn-primary btn-hover"
```

### 타입 안전한 환경 변수
```typescript
// 환경 변수 키 정의
type EnvPrefix = "REACT_APP" | "VITE" | "NEXT_PUBLIC";
type EnvKey<Prefix extends EnvPrefix, Name extends string> = 
  `${Prefix}_${Uppercase<Name>}`;

type ReactEnv = EnvKey<"REACT_APP", "api_url" | "api_key">;
// "REACT_APP_API_URL" | "REACT_APP_API_KEY"

type ViteEnv = EnvKey<"VITE", "port" | "host">;
// "VITE_PORT" | "VITE_HOST"

// 사용
function getEnv<Prefix extends EnvPrefix, Name extends string>(
  prefix: Prefix,
  name: Name
): string | undefined {
  const key = `${prefix}_${name.toUpperCase()}` as EnvKey<Prefix, Name>;
  return process.env[key];
}

const apiUrl = getEnv("REACT_APP", "api_url");
const port = getEnv("VITE", "port");
```

---

## 14.4 타입 가드 심화

타입 가드는 **런타임에서 타입을 좁혀주는** 함수입니다.

### 사용자 정의 타입 가드

**기본 문법:**
```typescript
function isType(value: unknown): value is Type {
  // 타입 체크 로직
  return true/false;
}
```

> 💡 **value is Type**: 이 함수가 true를 반환하면 value는 Type입니다

**예제 1: 기본 타입 가드**
```typescript
// 문자열 체크
function isString(value: unknown): value is string {
  return typeof value === "string";
}

// 숫자 체크
function isNumber(value: unknown): value is number {
  return typeof value === "number";
}

// 사용
function processValue(value: unknown) {
  if (isString(value)) {
    console.log(value.toUpperCase());  // ✅ string 메서드 사용 가능
  }
  
  if (isNumber(value)) {
    console.log(value.toFixed(2));  // ✅ number 메서드 사용 가능
  }
}
```

**예제 2: 객체 타입 가드**
```typescript
interface User {
  id: number;
  name: string;
  email: string;
}

interface Product {
  id: number;
  title: string;
  price: number;
}

// User 타입 가드
function isUser(obj: unknown): obj is User {
  return (
    typeof obj === "object" &&
    obj !== null &&
    "name" in obj &&
    "email" in obj &&
    typeof (obj as User).name === "string" &&
    typeof (obj as User).email === "string"
  );
}

// Product 타입 가드
function isProduct(obj: unknown): obj is Product {
  return (
    typeof obj === "object" &&
    obj !== null &&
    "title" in obj &&
    "price" in obj &&
    typeof (obj as Product).title === "string" &&
    typeof (obj as Product).price === "number"
  );
}

// 사용
function processData(data: unknown) {
  if (isUser(data)) {
    console.log(`User: ${data.name}, ${data.email}`);
    // data는 User 타입
  } else if (isProduct(data)) {
    console.log(`Product: ${data.title}, $${data.price}`);
    // data는 Product 타입
  }
}
```

**예제 3: 배열 타입 가드**
```typescript
function isStringArray(value: unknown): value is string[] {
  return Array.isArray(value) && value.every(item => typeof item === "string");
}

function isNumberArray(value: unknown): value is number[] {
  return Array.isArray(value) && value.every(item => typeof item === "number");
}

// 사용
function processArray(arr: unknown) {
  if (isStringArray(arr)) {
    arr.forEach(str => console.log(str.toUpperCase()));
    // arr은 string[]
  }
  
  if (isNumberArray(arr)) {
    const sum = arr.reduce((a, b) => a + b, 0);
    // arr은 number[]
  }
}
```

### is 키워드

`is` 키워드는 타입 가드 함수의 반환 타입에 사용됩니다.

**예제 1: API 응답 검증**
```typescript
interface SuccessResponse {
  success: true;
  data: any;
}

interface ErrorResponse {
  success: false;
  error: string;
}

type ApiResponse = SuccessResponse | ErrorResponse;

function isSuccessResponse(response: ApiResponse): response is SuccessResponse {
  return response.success === true;
}

function isErrorResponse(response: ApiResponse): response is ErrorResponse {
  return response.success === false;
}

// 사용
async function fetchData() {
  const response: ApiResponse = await fetch("/api/data").then(r => r.json());
  
  if (isSuccessResponse(response)) {
    console.log(response.data);  // ✅ data 속성 접근 가능
  } else if (isErrorResponse(response)) {
    console.error(response.error);  // ✅ error 속성 접근 가능
  }
}
```

**예제 2: 유니온 타입 좁히기**
```typescript
type Shape = 
  | { kind: "circle"; radius: number }
  | { kind: "square"; sideLength: number }
  | { kind: "rectangle"; width: number; height: number };

function isCircle(shape: Shape): shape is Extract<Shape, { kind: "circle" }> {
  return shape.kind === "circle";
}

function isSquare(shape: Shape): shape is Extract<Shape, { kind: "square" }> {
  return shape.kind === "square";
}

function isRectangle(shape: Shape): shape is Extract<Shape, { kind: "rectangle" }> {
  return shape.kind === "rectangle";
}

// 사용
function getArea(shape: Shape): number {
  if (isCircle(shape)) {
    return Math.PI * shape.radius ** 2;
    // shape은 { kind: "circle"; radius: number }
  }
  
  if (isSquare(shape)) {
    return shape.sideLength ** 2;
    // shape은 { kind: "square"; sideLength: number }
  }
  
  if (isRectangle(shape)) {
    return shape.width * shape.height;
    // shape은 { kind: "rectangle"; width: number; height: number }
  }
  
  return 0;
}
```

**예제 3: null/undefined 체크**
```typescript
function isDefined<T>(value: T | null | undefined): value is T {
  return value !== null && value !== undefined;
}

// 사용
function processValue(value: string | null | undefined) {
  if (isDefined(value)) {
    console.log(value.toUpperCase());  // ✅ string 확정
  }
}

// 배열 필터링
const values: (string | null | undefined)[] = ["a", null, "b", undefined, "c"];
const definedValues = values.filter(isDefined);
// definedValues는 string[] 타입
```

### asserts 키워드

`asserts` 키워드는 **assertion 함수**를 만들 때 사용합니다. 함수가 에러를 던지지 않으면 타입이 보장됩니다.

**기본 문법:**
```typescript
function assert(condition: boolean, message: string): asserts condition {
  if (!condition) {
    throw new Error(message);
  }
}
```

**예제 1: 타입 단언**
```typescript
function assertIsString(value: unknown): asserts value is string {
  if (typeof value !== "string") {
    throw new Error("Not a string!");
  }
}

function processValue(value: unknown) {
  assertIsString(value);
  // 이 줄 이후로 value는 string 타입
  console.log(value.toUpperCase());  // ✅ OK
}
```

**예제 2: null 체크**
```typescript
function assertNotNull<T>(value: T | null | undefined): asserts value is T {
  if (value === null || value === undefined) {
    throw new Error("Value is null or undefined!");
  }
}

function getUser(id: number): User | null {
  // 사용자 조회...
  return null;
}

function processUser(id: number) {
  const user = getUser(id);
  assertNotNull(user);
  // user는 이제 User 타입 (null 아님)
  console.log(user.name);  // ✅ OK
}
```

**예제 3: 객체 속성 확인**
```typescript
function assertHasProperty<T, K extends string>(
  obj: T,
  key: K,
  message?: string
): asserts obj is T & Record<K, unknown> {
  if (!(key in obj)) {
    throw new Error(message || `Property ${key} does not exist`);
  }
}

function processData(data: unknown) {
  assertHasProperty(data, "id");
  assertHasProperty(data, "name");
  
  // data는 이제 { id: unknown; name: unknown } 타입
  console.log(data.id, data.name);  // ✅ OK
}
```

**예제 4: 배열 검증**
```typescript
function assertIsArray<T>(value: unknown): asserts value is T[] {
  if (!Array.isArray(value)) {
    throw new Error("Not an array!");
  }
}

function assertIsStringArray(value: unknown): asserts value is string[] {
  assertIsArray(value);
  if (!value.every(item => typeof item === "string")) {
    throw new Error("Not a string array!");
  }
}

function processStrings(data: unknown) {
  assertIsStringArray(data);
  // data는 이제 string[] 타입
  data.forEach(str => console.log(str.toUpperCase()));  // ✅ OK
}
```

### is vs asserts 비교
```typescript
// is: 반환 값으로 타입 체크
function isString(value: unknown): value is string {
  return typeof value === "string";
}

// 사용: if문 필요
if (isString(value)) {
  console.log(value.toUpperCase());
}

// asserts: 에러를 던지거나 통과
function assertIsString(value: unknown): asserts value is string {
  if (typeof value !== "string") {
    throw new Error("Not a string");
  }
}

// 사용: if문 불필요
assertIsString(value);
console.log(value.toUpperCase());  // 여기서 value는 string

// 💡 언제 무엇을 사용할까?
// - is: 타입이 아닐 때 다른 처리가 필요한 경우
// - asserts: 타입이 아니면 에러를 던지는 경우
```

### 실전 예제: 폼 유효성 검사
```typescript
interface FormData {
  username: string;
  email: string;
  age: number;
}

function assertValidUsername(username: unknown): asserts username is string {
  if (typeof username !== "string") {
    throw new Error("Username must be a string");
  }
  if (username.length < 3) {
    throw new Error("Username must be at least 3 characters");
  }
  if (!/^[a-zA-Z0-9_]+$/.test(username)) {
    throw new Error("Username must contain only letters, numbers, and underscores");
  }
}

function assertValidEmail(email: unknown): asserts email is string {
  if (typeof email !== "string") {
    throw new Error("Email must be a string");
  }
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    throw new Error("Invalid email format");
  }
}

function assertValidAge(age: unknown): asserts age is number {
  if (typeof age !== "number") {
    throw new Error("Age must be a number");
  }
  if (age < 0 || age > 150) {
    throw new Error("Age must be between 0 and 150");
  }
}

function validateFormData(data: unknown): asserts data is FormData {
  if (typeof data !== "object" || data === null) {
    throw new Error("Invalid form data");
  }
  
  const { username, email, age } = data as any;
  
  assertValidUsername(username);
  assertValidEmail(email);
  assertValidAge(age);
}

// 사용
function submitForm(data: unknown) {
  try {
    validateFormData(data);
    // data는 이제 FormData 타입
    console.log(`Valid form: ${data.username}, ${data.email}, ${data.age}`);
  } catch (error) {
    console.error("Validation failed:", error);
  }
}
```

---

## 14.5 타입 단언 (Type Assertion)

타입 단언은 **컴파일러에게 "내가 타입을 더 잘 알아"**라고 알려주는 것입니다.

### as 키워드

**기본 문법:**
```typescript
value as Type
```

**예제 1: DOM 요소**
```typescript
// HTMLElement → HTMLInputElement
const input = document.getElementById("username") as HTMLInputElement;
input.value = "홍길동";  // ✅ OK

// HTMLElement → HTMLButtonElement
const button = document.querySelector(".submit-btn") as HTMLButtonElement;
button.disabled = true;  // ✅ OK

// 여러 타입 가능성
const element = document.querySelector(".some-element");
if (element) {
  // element는 Element | null
  const input = element as HTMLInputElement;
  console.log(input.value);
}
```

**예제 2: API 응답**
```typescript
interface User {
  id: number;
  name: string;
  email: string;
}

async function fetchUser(id: number): Promise<User> {
  const response = await fetch(`/api/users/${id}`);
  const data = await response.json();  // any 타입
  return data as User;  // User로 단언
}

// 사용
const user = await fetchUser(1);
console.log(user.name, user.email);  // ✅ OK
```

**예제 3: 타입 좁히기**
```typescript
type Animal = { type: "dog"; bark: () => void } | { type: "cat"; meow: () => void };

function handleAnimal(animal: Animal) {
  if (animal.type === "dog") {
    // TypeScript가 자동으로 좁혀주지 못할 때
    const dog = animal as { type: "dog"; bark: () => void };
    dog.bark();
  }
}
```

### 이중 단언 (Double Assertion)

때로는 두 번 단언해야 할 때가 있습니다:
```typescript
// ❌ 직접 변환 불가
const value = "hello" as number;  // 에러!

// ✅ unknown을 거쳐서 변환 (권장하지 않음!)
const value = "hello" as unknown as number;

// 💡 하지만 이렇게 하지 마세요!
// 런타임 에러가 발생할 수 있습니다
```

### const 단언

`as const`는 값을 **리터럴 타입**으로 만듭니다:
```typescript
// 일반 선언
const user1 = {
  name: "홍길동",
  age: 30
};
// 타입: { name: string; age: number }

// as const 사용
const user2 = {
  name: "홍길동",
  age: 30
} as const;
// 타입: { readonly name: "홍길동"; readonly age: 30 }

// 배열
const colors1 = ["red", "green", "blue"];
// 타입: string[]

const colors2 = ["red", "green", "blue"] as const;
// 타입: readonly ["red", "green", "blue"]
```

**실전 예제: 설정 객체**
```typescript
const config = {
  apiUrl: "https://api.example.com",
  timeout: 5000,
  retryCount: 3
} as const;

// config.apiUrl = "https://other.com";  // ❌ 에러! readonly

// 타입 추출
type Config = typeof config;
/*
{
  readonly apiUrl: "https://api.example.com";
  readonly timeout: 5000;
  readonly retryCount: 3;
}
*/

// 값으로 타입 생성
type ApiUrl = typeof config.apiUrl;  // "https://api.example.com"
```

**실전 예제: 라우트 정의**
```typescript
const routes = {
  home: "/",
  about: "/about",
  users: "/users",
  userDetail: "/users/:id"
} as const;

type Route = typeof routes[keyof typeof routes];
// "/" | "/about" | "/users" | "/users/:id"

function navigate(route: Route) {
  console.log(`Navigating to ${route}`);
}

navigate("/");  // ✅ OK
navigate("/about");  // ✅ OK
navigate("/invalid");  // ❌ 에러!
```

### 주의사항

**❌ 피해야 할 사용:**
```typescript
// 1. 잘못된 타입 단언
const value = "123" as number;  // ❌ 런타임 에러 발생!
console.log(value * 2);  // NaN

// 2. any를 통한 우회
const value2 = "123" as any as number;  // ❌ 타입 안전성 포기

// 3. 불필요한 단언
const name: string = "홍길동" as string;  // ❌ 불필요

// 4. 과도한 단언
interface User {
  name: string;
}
const data = { name: "홍길동", age: 30 };
const user = data as User;  // ⚠️ age 속성이 무시됨
```

**✅ 올바른 사용:**
```typescript
// 1. DOM 요소
const input = document.getElementById("input") as HTMLInputElement;

// 2. API 응답 (검증 후)
const data = await response.json();
if (isValidUser(data)) {
  const user = data as User;
}

// 3. 리터럴 타입
const colors = ["red", "green", "blue"] as const;

// 4. 타입 좁히기 보조
if (animal.type === "dog") {
  (animal as Dog).bark();
}
```

