...
프로퍼티 데코레이터는 **클래스 속성에 메타데이터를 추가**합니다.

**문법:**
```typescript
function PropertyDecorator(
  target: any,                    // 클래스의 prototype (인스턴스 속성) 또는 생성자 (정적 속성)
  propertyKey: string | symbol    // 속성 이름
) {
  // 속성 자체는 수정할 수 없음 (descriptor가 없음)
  // 대신 메타데이터를 저장
}
...
const user1 = new User("홍길동", 30, "hong@example.com");
console.log(validateType(user1));  // true

const user2 = new User("김철수", 25, "invalid-email");
console.log(validateType(user2));  // false
// 출력: email must be a valid email
```
...
매개변수 데코레이터는 **메서드의 매개변수에 메타데이터를 추가**합니다.

**문법:**
```typescript
function ParameterDecorator(
  target: any,                    // 클래스의 prototype
  propertyKey: string | symbol,  // 메서드 이름
  parameterIndex: number          // 매개변수의 위치 (0부터 시작)
) {
  // 매개변수 정보를 메타데이터로 저장
}
...
callWithValidation(service, 'createUser', '홍길동', 'hong@example.com', 30);  // ✅ OK
callWithValidation(service, 'createUser', '', 'hong@example.com');  // ❌ 에러
// 출력: Parameter at index 0 is required
```
...
const user = callWithInjection(userService, 'getUser', 1);  // repo와 logger는 자동 주입
console.log(user);  // { id: 1, name: 'User' }
```

### 예제 2: 타입 검증
typescriptfunction ValidateType(expectedType: string) {
  return function(target: any, propertyKey: string, parameterIndex: number) {
...
console.log(validateAndCall(calc, 'add', 5, 3));  // ✅ 8
console.log(validateAndCall(calc, 'add', '5', 3));  // ❌ 에러!
```
...
데코레이터 팩토리는 설정을 받아서 데코레이터를 반환하는 함수입니다.
...
console.log(service.getUser(1));  // Cache hit (5초 내)
setTimeout(() => {
  console.log(service.getUser(1));  // Cache miss (5초 후)
}, 6000);
```
...
function RequireRole(...roles: string[]) {
  return function(
    ...
    descriptor.value = function(this: any, ...args: any[]) {
      // 현재 사용자 역할 가져오기 (this.currentUser를 가정)
...
adminService.deleteUser(100);  // ✅ OK
adminService.banUser(200);     // ✅ OK

// 일반 사용자로 로그인
adminService.currentUser = { id: 2, role: 'user' };
adminService.deleteUser(100);  // ❌ 에러: Access denied
예제 4: 성능 임계값
typescriptinterface PerformanceConfig {
  threshold: number;  // 밀리초
...
service.processData(10);       // 빠름, 경고 없음
```
service.processData(10000000); // 느림, 경고 발생
