...
async function main(): Promise<void> {
  ...
  const task3Dto: CreateTaskDto = {
    ...
    priority: 'urgent',
    assigneeId: userId1,
    dueDate: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000), // 1일 전 (overdue)
    tags: ['bug', 'critical'],
  };
...
### 9-1. tests/user-service.test.ts (예제)
```typescript
import 'reflect-metadata';
import { UserService } from '@services/user-service';
import { CreateUserDto } from '@models/user';
...
describe('UserService', () => {
  ...
  test('should create user successfully', async () => {
    ...
    if (result.success) {
      ...
      expect(result.data).not.toHaveProperty('password'); // password 제외 확인
    }
  });
...
## 📊 Step 11: 프로젝트 확장 아이디어

이제 기본 프로젝트가 완성되었습니다! 다음과 같이 확장할 수 있습니다:
...
### 확장 기능 5: RESTful API (Express 통합)
```typescript
...
### Chapter 13: 유틸리티 타입
```
✅ Omit: CreateUserDto, UpdateUserDto
✅ Partial: UpdateTaskDto
✅ Pick: UserProfile (password 제외)
✅ Record: byStatus, byPriority 통계
✅ 커스텀 유틸리티: Optional, DeepPartial, PickByType
```
...
✅ 조건부 타입: TaskByStatus
✅ 매핑 타입: 커스텀 유틸리티 타입
✅ 템플릿 리터럴 타입: API 응답 메시지
✅ 타입 가드: ValidationError
✅ 제네릭: BaseRepository, ApiResponse
```

### Chapter 15: 모듈 시스템
```
✅ ES Modules: import/export
✅ 경로 별칭: @models, @services, @decorators
✅ 모듈 구조화: models, repositories, services 분리
✅ index.ts로 re-export
```

### Chapter 16: 데코레이터
```
✅ 메서드 데코레이터: @Log, @LogAsync, @Validate
✅ 프로퍼티 데코레이터: @Required, @Email, @MinLength
✅ 데코레이터 팩토리: @Cache({ ttl: 5000 })
✅ reflect-metadata 활용
```

### Chapter 17: TypeScript 설정
```
✅ tsconfig.json 설정
✅ 경로 별칭 설정
✅ ESLint + Prettier
...
**프로젝트 특징:**
- Part 3의 모든 개념 적용
- 실전 아키텍처 (Repository-Service 패턴)
- 데코레이터 활용 (로깅, 유효성 검사, 캐싱)
- 유틸리티 타입 활용
- 모듈 시스템 구조화
- TypeScript 설정 최적화
- 확장 가능한 구조

실제로 실행해볼 수 있는 완전한 프로젝트입니다! 
