---
title: "[TS] 유틸리티 타입 - 3"
date: 2025-10-21 16:00:00 +0900
categories: [웹 개발, 타입스크립트]
tags: [웹개발, 타입스크립트, TS, 유틸리티타입]
pin: true
math: true
mermaid: true
---


> **학습목표**
> - 유틸리티의 필요성을 이해한다
> - 타입을 추출하고 변형할 수 있다.
> - 유니온 타입을 조작할 수 있다.
> - 함수 타입을 다룰 수 있다.
> - 고급 패턴을 적용할 수 있다.

# 13.8 Exclude와 Extract

## Exclude<T, U>

### 개념

유니온 타입 T에서 U에 할당 가능한 타입을 제외합니다.

**문법:**
```typescript
type Exclude<T, U> = T extends U ? never : T;
```

### 기본 사용법:
```typescript
type AllTypes = 'a' | 'b' | 'c' | 'd';

// 'b'와 'c' 제외
type ExcludedTypes = Exclude<AllTypes, 'b' | 'c'>;
// 결과: 'a' | 'd'

const value1: ExcludedTypes = 'a'; // ✅ OK
const value2: ExcludedTypes = 'd'; // ✅ OK
const value3: ExcludedTypes = 'b'; // ❌ 에러!
```

### 실전 예제 

**1: 타입 필터링**
```typescript
type Primitive = string | number | boolean | null | undefined;

// null과 undefined 제외
type NonNullablePrimitive = Exclude<Primitive, null | undefined>;
// 결과: string | number | boolean

const val1: NonNullablePrimitive = "hello"; // ✅ OK
const val2: NonNullablePrimitive = null; // ❌ 에러!
```

**2: 이벤트 타입 필터링**
```typescript
type MouseEvent = 'click' | 'mousedown' | 'mouseup' | 'mousemove';
type KeyboardEvent = 'keydown' | 'keyup' | 'keypress';
type AllEvents = MouseEvent | KeyboardEvent;

// 마우스 이벤트만 제외
type OnlyKeyboardEvents = Exclude<AllEvents, MouseEvent>;
// 결과: 'keydown' | 'keyup' | 'keypress'

function handleKeyboardEvent(event: OnlyKeyboardEvents) {
  console.log(`Keyboard event: ${event}`);
}

handleKeyboardEvent('keydown'); // ✅ OK
handleKeyboardEvent('click'); // ❌ 에러!
```

**3: 함수 타입 제외**
```typescript
type MixedType = string | number | (() => void) | boolean;

// 함수 타입 제외
type NonFunctionTypes = Exclude<MixedType, Function>;
// 결과: string | number | boolean

const value: NonFunctionTypes = "text"; // ✅ OK
const value2: NonFunctionTypes = () => {}; // ❌ 에러!
```

## Extract<T, U>

### 개념

유니온 타입 T에서 U에 할당 가능한 타입만 추출합니다. Exclude의 반대입니다.

**문법:**
```typescript
type Extract<T, U> = T extends U ? T : never;
```

### 기본 사용법:
```typescript
type AllTypes = 'a' | 'b' | 'c' | 'd';

// 'b'와 'c'만 추출
type ExtractedTypes = Extract<AllTypes, 'b' | 'c'>;
// 결과: 'b' | 'c'

const value1: ExtractedTypes = 'b'; // ✅ OK
const value2: ExtractedTypes = 'c'; // ✅ OK
const value3: ExtractedTypes = 'a'; // ❌ 에러!
```

###실전 예제 

**1: 특정 타입만 추출**
```typescript
type MixedType = string | number | boolean | (() => void);

// 함수 타입만 추출
type OnlyFunctions = Extract<MixedType, Function>;
// 결과: () => void

const fn: OnlyFunctions = () => console.log("Hello"); // ✅ OK
const fn2: OnlyFunctions = "text"; // ❌ 에러!

// 원시 타입만 추출
type OnlyPrimitives = Extract<MixedType, string | number | boolean>;
// 결과: string | number | boolean
```

**2: 공통 속성 추출**
```typescript
type EventType = 'click' | 'focus' | 'blur' | 'submit';
type FormEventType = 'focus' | 'blur' | 'submit' | 'change';

// 공통 이벤트만 추출
type CommonEvents = Extract<EventType, FormEventType>;
// 결과: 'focus' | 'blur' | 'submit'

function handleCommonEvent(event: CommonEvents) {
  console.log(`Common event: ${event}`);
}

handleCommonEvent('focus'); // ✅ OK
handleCommonEvent('click'); // ❌ 에러!
```

**3: 객체 키 타입 필터링**
```typescript
interface User {
  id: number;
  name: string;
  email: string;
  age: number;
  isActive: boolean;
}

// 숫자 타입 속성만 추출
type NumberKeys = Extract<keyof User, 'id' | 'age'>;
// 결과: 'id' | 'age'

// 하지만 더 나은 방법은 (13.12에서 다룹니다):
type NumberKeysAuto = {
  [K in keyof User]: User[K] extends number ? K : never;
}[keyof User];
Exclude vs Extract 비교
typescripttype All = 'a' | 'b' | 'c' | 'd' | 'e';
type Some = 'b' | 'd';

// Exclude: 제외 (차집합)
type WithoutSome = Exclude<All, Some>;
// 'a' | 'c' | 'e'

// Extract: 추출 (교집합)
type OnlySome = Extract<All, Some>;
// 'b' | 'd'

// 💡 시각화
// All:     [a, b, c, d, e]
// Some:         [b,    d]
// Exclude: [a,    c,    e]  (Some 제외)
// Extract:      [b,    d]   (Some만 추출)
```

## 13.9 NonNullable<T>

### 개념

NonNullable<T>는 타입 T에서 null과 undefined를 제거합니다.

**문법:**
```typescript
type NonNullable<T> = T extends null | undefined ? never : T;
// 또는
type NonNullable<T> = Exclude<T, null | undefined>;
```

### 기본 사용법
```typescript
type MaybeString = string | null | undefined;

// null과 undefined 제거
type DefinitelyString = NonNullable<MaybeString>;
// 결과: string

const value1: DefinitelyString = "hello"; // ✅ OK
const value2: DefinitelyString = null; // ❌ 에러!
const value3: DefinitelyString = undefined; // ❌ 에러!
```

### 실전 활용 예제

**1. 옵셔널 값 처리**
```typescript
interface User {
  id: number;
  name: string;
  email?: string; // 선택적
  phone?: string; // 선택적
}

function getUser(): User {
  return {
    id: 1,
    name: "홍길동",
    email: "hong@example.com"
    // phone은 없음
  };
}

// 이메일이 있는 경우만 처리
function sendEmail(email: NonNullable<User['email']>) {
  console.log(`Sending email to: ${email}`);
  // email은 확실히 string (undefined 아님)
}

const user = getUser();

if (user.email) {
  sendEmail(user.email); // ✅ OK (타입 가드로 체크)
}

// sendEmail(user.phone); // ❌ 에러! phone은 undefined일 수 있음
```

**2. 배열 필터링**
```typescript
const numbers: (number | null | undefined)[] = [1, 2, null, 3, undefined, 4];

// null과 undefined 제거
function filterNullable<T>(arr: T[]): NonNullable<T>[] {
  return arr.filter((item): item is NonNullable<T> => item != null);
}

const validNumbers = filterNullable(numbers);
// 타입: number[]
// 값: [1, 2, 3, 4]

console.log(validNumbers); // [1, 2, 3, 4]
```

**3. API 응답 처리**
```typescript
interface ApiResponse<T> {
  data: T | null;
  error: string | null;
}

function getUser(id: number): ApiResponse<User> {
  // API 호출 시뮬레이션
  return {
    data: { id, name: "홍길동", email: "hong@example.com" },
    error: null
  };
}

// 성공 응답 처리
function processUserData(data: NonNullable<ApiResponse<User>['data']>) {
  // data는 확실히 User (null 아님)
  console.log(`User: ${data.name}, ${data.email}`);
}

const response = getUser(1);

if (response.data) {
  processUserData(response.data); // ✅ 타입 안전
}
```

**4. 설정 값 보장**
```typescript
interface Config {
  apiUrl?: string;
  timeout?: number;
  retryCount?: number;
}

const userConfig: Config = {
  apiUrl: "https://api.example.com"
  // timeout과 retryCount는 없음
};

// 기본값과 병합하여 NonNullable 보장
function getCompleteConfig(config: Config): Required<Config> {
  return {
    apiUrl: config.apiUrl ?? "https://default.com",
    timeout: config.timeout ?? 5000,
    retryCount: config.retryCount ?? 3
  };
}

const completeConfig = getCompleteConfig(userConfig);
// completeConfig의 모든 속성은 확실히 존재

console.log(completeConfig.timeout); // 5000 (undefined 아님)
```

## 13.10 ReturnType<T>

### 개념

ReturnType<T>는 함수 타입 T의 반환 타입을 추출합니다.

**문법:**
```typescript
type ReturnType<T extends (...args: any) => any> = T extends (...args: any) => infer R ? R : any;

💡 infer R: 반환 타입을 추론하여 R에 저장
```

### 기본 사용법
```typescript
function getUser() {
  return {
    id: 1,
    name: "홍길동",
    email: "hong@example.com"
  };
}

// getUser 함수의 반환 타입 추출
type User = ReturnType<typeof getUser>;

/*
결과:
type User = {
  id: number;
  name: string;
  email: string;
}
*/

const user: User = {
  id: 2,
  name: "김철수",
  email: "kim@example.com"
}; // ✅ OK
```

### 실전 활용 예제

**1. API 응답 타입 추출**
```typescript
async function fetchUserData() {
  const response = await fetch('/api/user');
  return response.json();
}

// 반환 타입 추출
type UserData = ReturnType<typeof fetchUserData>;
// 결과: Promise<any>

// Promise를 풀어내려면 Awaited 사용 (TypeScript 4.5+)
type UnwrappedUserData = Awaited<ReturnType<typeof fetchUserData>>;
```

**2. 팩토리 함수의 반환 타입**
```typescript
function createUser(name: string, email: string) {
  return {
    id: Date.now(),
    name,
    email,
    createdAt: new Date(),
    isActive: true
  };
}

// 타입을 별도로 정의하지 않고 함수에서 추출
type User = ReturnType<typeof createUser>;

/*
type User = {
  id: number;
  name: string;
  email: string;
  createdAt: Date;
  isActive: boolean;
}
*/

function saveUser(user: User) {
  console.log(`Saving user: ${user.name}`);
}

const newUser = createUser("홍길동", "hong@example.com");
saveUser(newUser); // ✅ 타입 안전
```

**3. 상태 관리 함수**
```typescript
function createStore() {
  let state = {
    count: 0,
    message: "Hello"
  };
  
  return {
    getState: () => state,
    increment: () => state.count++,
    setMessage: (msg: string) => { state.message = msg; }
  };
}

type Store = ReturnType<typeof createStore>;
type StoreState = ReturnType<Store['getState']>;

/*
type Store = {
  getState: () => { count: number; message: string; };
  increment: () => void;
  setMessage: (msg: string) => void;
}

type StoreState = {
  count: number;
  message: string;
}
*/

function useStore(store: Store) {
  const state: StoreState = store.getState();
  console.log(state.count, state.message);
}
```

**4. 제네릭 함수의 반환 타입**
```typescript
function createArray<T>(value: T, count: number) {
  return Array(count).fill(value);
}

// 제네릭 함수의 반환 타입
type StringArray = ReturnType<typeof createArray<string>>;
// 결과: string[]

type NumberArray = ReturnType<typeof createArray<number>>;
// 결과: number[]

const strings: StringArray = createArray("hello", 3);
// ["hello", "hello", "hello"]
```

## 13.11 Parameters<T>

### 개념

Parameters<T>는 함수 타입 T의 매개변수 타입들을 튜플로 추출합니다.

**문법:**
```typescript
type Parameters<T extends (...args: any) => any> = T extends (...args: infer P) => any ? P : never;

💡 infer P: 매개변수 타입들을 추론하여 P에 저장 (튜플 형태)
```

### 기본 사용법
```typescript
function createUser(name: string, age: number, email: string) {
  return { name, age, email };
}

// createUser 함수의 매개변수 타입 추출
type CreateUserParams = Parameters<typeof createUser>;
// 결과: [name: string, age: number, email: string]

// 사용
const params: CreateUserParams = ["홍길동", 30, "hong@example.com"];
const user = createUser(...params); // ✅ OK
```

### 실전 활용 예제

**1. 함수 래퍼**
```typescript
function logUser(id: number, name: string, isActive: boolean) {
  console.log(`User #${id}: ${name} (Active: ${isActive})`);
}

// 로깅 기능을 추가하는 래퍼
function withLogging(
  fn: typeof logUser,
  ...args: Parameters<typeof logUser>
) {
  console.log(`[LOG] Calling function with args:`, args);
  fn(...args);
  console.log(`[LOG] Function completed`);
}

// 사용
withLogging(logUser, 1, "홍길동", true);
```

**2. 이벤트 핸들러 타입**
```typescript
function handleClick(event: MouseEvent, elementId: string) {
  console.log(`Clicked element: ${elementId}`);
}

// 이벤트 핸들러 매개변수 타입
type ClickHandlerParams = Parameters<typeof handleClick>;
// [event: MouseEvent, elementId: string]

function addEventListener(
  element: HTMLElement,
  handler: typeof handleClick
) {
  element.onclick = (event) => {
    const params: ClickHandlerParams = [event, element.id];
    handler(...params);
  };
}
```

**3. API 호출 함수 타입 안전성**
```typescript
async function fetchUser(userId: number, includeDeleted: boolean = false) {
  const response = await fetch(`/api/users/${userId}?deleted=${includeDeleted}`);
  return response.json();
}

// 매개변수 타입을 재사용
type FetchUserParams = Parameters<typeof fetchUser>;

function cacheableFetchUser(...args: FetchUserParams) {
  const cacheKey = JSON.stringify(args);
  const cached = getFromCache(cacheKey);
  
  if (cached) {
    return Promise.resolve(cached);
  }
  
  return fetchUser(...args).then(data => {
    saveToCache(cacheKey, data);
    return data;
  });
}

// 사용
cacheableFetchUser(1, true); // ✅ 타입 안전
```

**4. 함수 시그니처 복사**
```typescript
function originalFunction(a: string, b: number, c: boolean): void {
  console.log(a, b, c);
}

// 같은 시그니처를 가진 함수 타입
type SameSignature = (...args: Parameters<typeof originalFunction>) => ReturnType<typeof originalFunction>;

const copiedFunction: SameSignature = (a, b, c) => {
  console.log("Copied:", a, b, c);
};

copiedFunction("hello", 42, true); // ✅ OK
Parameters와 ReturnType 조합
typescriptfunction calculate(a: number, b: number, operation: 'add' | 'subtract'): number {
  return operation === 'add' ? a + b : a - b;
}

// 매개변수와 반환 타입 모두 추출
type CalculateParams = Parameters<typeof calculate>;
type CalculateReturn = ReturnType<typeof calculate>;

// 같은 시그니처의 새 함수
function loggedCalculate(...args: CalculateParams): CalculateReturn {
  console.log(`Calculating with args:`, args);
  const result = calculate(...args);
  console.log(`Result: ${result}`);
  return result;
}

loggedCalculate(10, 5, 'add'); // Calculating with args: [10, 5, 'add']
                                // Result: 15
```
