---
title: "[Projects] 도서관 관리 시스템 - 3"
date: 2025-10-15 16:30:00 +0900
categories: [웹 개발, 타입스크립트]
tags: [웹개발, 타입스크립트, TS]
pin: true
math: true
mermaid: true
published: false
---

# Project 2: 도서관 관리 시스템

> **프로젝트 목표**
> - 클래스 상속과 추상 클래스를 실전에 적용한다
> - 복잡한 비즈니스 로직을 설계하고 구현한다
> - 여러 엔티티 간의 관계를 관리한다
> - 날짜 계산 및 연체 처리를 구현한다
> - AI와 협업하되 더 자율적으로 프로젝트를 진행한다

---

---

## 부록: 완성 코드 구조 예시

### 📌 프로젝트 구조
```
library-system/
├── src/
│   ├── types/
│   │   ├── book.types.ts
│   │   ├── member.types.ts
│   │   └── loan.types.ts
│   ├── models/
│   │   ├── Book.ts
│   │   ├── Member.ts           # 방법 A
│   │   ├── AbstractMember.ts   # 방법 B
│   │   ├── RegularMember.ts    # 방법 B
│   │   ├── VIPMember.ts        # 방법 B
│   │   └── Loan.ts
│   ├── managers/
│   │   └── LibraryManager.ts
│   ├── utils/
│   │   ├── dateUtils.ts
│   │   └── helpers.ts
│   └── index.ts
├── tests/
│   ├── Book.test.ts
│   ├── Member.test.ts
│   ├── Loan.test.ts
│   └── LibraryManager.test.ts
├── package.json
├── tsconfig.json
└── README.md
```

### 📝 핵심 코드 스니펫

**types/book.types.ts**
```typescript
/**
 * 책 정보를 나타내는 인터페이스
 */
export interface IBook {
  /** ISBN (국제 표준 도서 번호) */
  readonly isbn: string;
  /** 제목 */
  title: string;
  /** 저자 */
  author: string;
  /** 출판 연도 */
  publishedYear: number;
  /** 대출 가능 여부 */
  isAvailable: boolean;
  /** 카테고리 (선택적) */
  category?: BookCategory;
}

/**
 * 책 카테고리
 */
export type BookCategory = "fiction" | "non-fiction" | "science" | "history" | "other";
```

**models/Book.ts**
```typescript
import { IBook, BookCategory } from "../types/book.types";

/**
 * 책 클래스
 */
export class Book implements IBook {
  constructor(
    public readonly isbn: string,
    public title: string,
    public author: string,
    public publishedYear: number,
    public isAvailable: boolean = true,
    public category?: BookCategory
  ) {}

  /**
   * 책 정보를 문자열로 반환
   */
  getInfo(): string {
    return `[${this.isbn}] ${this.title} - ${this.author} (${this.publishedYear})`;
  }

  /**
   * 대출 처리
   * @returns 성공 여부
   */
  checkout(): boolean {
    if (!this.isAvailable) {
      console.log(`❌ "${this.title}"은(는) 이미 대출 중입니다.`);
      return false;
    }
    this.isAvailable = false;
    return true;
  }

  /**
   * 반납 처리
   * @returns 성공 여부
   */
  returnBook(): boolean {
    if (this.isAvailable) {
      console.log(`❌ "${this.title}"은(는) 대출 중이 아닙니다.`);
      return false;
    }
    this.isAvailable = true;
    return true;
  }
}
```

**models/AbstractMember.ts (방법 B - 상속 구조)**
```typescript
/**
 * 추상 회원 클래스
 */
export abstract class Member {
  protected borrowedBooks: string[] = [];
  protected readonly registeredDate: Date;

  constructor(
    protected readonly id: string,
    protected name: string
  ) {
    this.registeredDate = new Date();
  }

  /**
   * 회원 ID 반환
   */
  getId(): string {
    return this.id;
  }

  /**
   * 회원 이름 반환
   */
  getName(): string {
    return this.name;
  }

  /**
   * 대출 가능 여부 확인
   */
  canBorrow(): boolean {
    return this.getBorrowedCount() < this.getMaxBorrowLimit();
  }

  /**
   * 대출 중인 책 추가
   */
  addBorrowedBook(isbn: string): void {
    if (!this.borrowedBooks.includes(isbn)) {
      this.borrowedBooks.push(isbn);
    }
  }

  /**
   * 대출 중인 책 제거
   */
  removeBorrowedBook(isbn: string): boolean {
    const index = this.borrowedBooks.indexOf(isbn);
    if (index === -1) {
      return false;
    }
    this.borrowedBooks.splice(index, 1);
    return true;
  }

  /**
   * 현재 대출 중인 책 개수
   */
  getBorrowedCount(): number {
    return this.borrowedBooks.length;
  }

  /**
   * 대출 중인 책 목록
   */
  getBorrowedBooks(): string[] {
    return [...this.borrowedBooks];
  }

  /**
   * 최대 대출 가능 권수 (추상 메서드)
   */
  abstract getMaxBorrowLimit(): number;

  /**
   * 회원 등급 (추상 메서드)
   */
  abstract getMemberType(): string;
}
```

**models/RegularMember.ts**
```typescript
import { Member } from "./AbstractMember";

/**
 * 일반 회원 클래스
 */
export class RegularMember extends Member {
  constructor(id: string, name: string) {
    super(id, name);
  }

  getMaxBorrowLimit(): number {
    return 3;
  }

  getMemberType(): string {
    return "일반 회원";
  }
}
```

**models/VIPMember.ts**
```typescript
import { Member } from "./AbstractMember";

/**
 * 우수 회원 클래스
 */
export class VIPMember extends Member {
  constructor(id: string, name: string) {
    super(id, name);
  }

  getMaxBorrowLimit(): number {
    return 5;
  }

  getMemberType(): string {
    return "우수 회원";
  }
}
```

**models/Loan.ts**
```typescript
/**
 * 대출 클래스
 */
export class Loan {
  private readonly borrowDate: Date;
  private readonly dueDate: Date;
  private returnDate: Date | null = null;

  constructor(
    public readonly id: string,
    public readonly bookIsbn: string,
    public readonly memberId: string
  ) {
    this.borrowDate = new Date();
    this.dueDate = this.calculateDueDate(this.borrowDate, 14);
  }

  /**
   * 반납 예정일 계산
   */
  private calculateDueDate(borrowDate: Date, days: number): Date {
    const dueDate = new Date(borrowDate);
    dueDate.setDate(dueDate.getDate() + days);
    return dueDate;
  }

  /**
   * 연체 여부 확인
   */
  isOverdue(): boolean {
    if (this.returnDate !== null) {
      return false;
    }
    return new Date() > this.dueDate;
  }

  /**
   * 연체 일수 계산
   */
  getOverdueDays(): number {
    if (!this.isOverdue()) {
      return 0;
    }
    const today = new Date();
    const diffTime = today.getTime() - this.dueDate.getTime();
    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
    return diffDays;
  }

  /**
   * 반납 처리
   */
  returnBook(): void {
    this.returnDate = new Date();
  }

  /**
   * 반납 완료 여부
   */
  isReturned(): boolean {
    return this.returnDate !== null;
  }

  /**
   * 대출 정보 문자열
   */
  getLoanInfo(): string {
    const borrowDateStr = this.borrowDate.toLocaleDateString();
    const dueDateStr = this.dueDate.toLocaleDateString();
    return `회원 ${this.memberId}이(가) ${this.bookIsbn}을(를) ${borrowDateStr}에 대출 (반납 예정: ${dueDateStr})`;
  }

  /**
   * 대출일 반환
   */
  getBorrowDate(): Date {
    return new Date(this.borrowDate);
  }

  /**
   * 반납 예정일 반환
   */
  getDueDate(): Date {
    return new Date(this.dueDate);
  }

  /**
   * 반납일 반환
   */
  getReturnDate(): Date | null {
    return this.returnDate ? new Date(this.returnDate) : null;
  }
}
```

**managers/LibraryManager.ts (주요 메서드만)**
```typescript
import { Book } from "../models/Book";
import { Member } from "../models/AbstractMember";
import { Loan } from "../models/Loan";

/**
 * 도서관 통계 인터페이스
 */
export interface LibraryStatistics {
  totalBooks: number;
  availableBooks: number;
  borrowedBooks: number;
  totalMembers: number;
  activeLoans: number;
  overdueLoans: number;
  totalLoans: number;
}

/**
 * 도서관 관리 클래스
 */
export class LibraryManager {
  private books: Map<string, Book> = new Map();
  private members: Map<string, Member> = new Map();
  private loans: Map<string, Loan> = new Map();
  private nextLoanId: number = 1;

  /**
   * 책 추가
   */
  addBook(book: Book): void {
    if (this.books.has(book.isbn)) {
      throw new Error(`ISBN ${book.isbn}은(는) 이미 등록된 책입니다.`);
    }
    this.books.set(book.isbn, book);
    console.log(`✅ 책이 추가되었습니다: ${book.getInfo()}`);
  }

  /**
   * 회원 등록
   */
  registerMember(member: Member): void {
    if (this.members.has(member.getId())) {
      throw new Error(`회원 ID ${member.getId()}은(는) 이미 등록되어 있습니다.`);
    }
    this.members.set(member.getId(), member);
    console.log(`✅ 회원이 등록되었습니다: ${member.getName()} (${member.getMemberType()})`);
  }

  /**
   * 책 대출
   */
  borrowBook(memberId: string, isbn: string): Loan | null {
    // 1. 회원 확인
    const member = this.members.get(memberId);
    if (!member) {
      console.log(`❌ 회원 ID ${memberId}을(를) 찾을 수 없습니다.`);
      return null;
    }

    // 2. 책 확인
    const book = this.books.get(isbn);
    if (!book) {
      console.log(`❌ ISBN ${isbn}의 책을 찾을 수 없습니다.`);
      return null;
    }

    // 3. 대출 가능 여부 검증
    if (!book.isAvailable) {
      console.log(`❌ "${book.title}"은(는) 이미 대출 중입니다.`);
      return null;
    }

    if (!member.canBorrow()) {
      console.log(`❌ ${member.getName()}님은 대출 한도를 초과했습니다.`);
      return null;
    }

    // 4. 대출 처리
    const loanId = `L${String(this.nextLoanId++).padStart(3, "0")}`;
    const loan = new Loan(loanId, isbn, memberId);

    book.checkout();
    member.addBorrowedBook(isbn);
    this.loans.set(loanId, loan);

    console.log(`✅ 대출 완료: ${member.getName()}님이 "${book.title}"을(를) 대출했습니다.`);
    return loan;
  }

  /**
   * 책 반납
   */
  returnBook(loanId: string): boolean {
    // 1. 대출 기록 확인
    const loan = this.loans.get(loanId);
    if (!loan) {
      console.log(`❌ 대출 기록 ${loanId}을(를) 찾을 수 없습니다.`);
      return false;
    }

    // 2. 이미 반납 확인
    if (loan.isReturned()) {
      console.log(`❌ 이미 반납된 대출입니다.`);
      return false;
    }

    // 3. 반납 처리
    const book = this.books.get(loan.bookIsbn);
    const member = this.members.get(loan.memberId);

    if (!book || !member) {
      console.log(`❌ 반납 처리 중 오류가 발생했습니다.`);
      return false;
    }

    loan.returnBook();
    book.returnBook();
    member.removeBorrowedBook(loan.bookIsbn);

    // 4. 연체 확인
    if (loan.getOverdueDays() > 0) {
      const overdueDays = loan.getOverdueDays();
      const fine = overdueDays * 100;
      console.log(`⚠️ 연체: ${overdueDays}일 (연체료: ${fine}원)`);
    }

    console.log(`✅ 반납 완료: ${member.getName()}님이 "${book.title}"을(를) 반납했습니다.`);
    return true;
  }

  /**
   * 책 검색
   */
  searchBooks(query: string): Book[] {
    const lowerQuery = query.toLowerCase();
    return Array.from(this.books.values()).filter(
      book =>
        book.title.toLowerCase().includes(lowerQuery) ||
        book.author.toLowerCase().includes(lowerQuery)
    );
  }

  /**
   * 통계 조회
   */
  getStatistics(): LibraryStatistics {
    const allBooks = Array.from(this.books.values());
    const allLoans = Array.from(this.loans.values());
    const activeLoans = allLoans.filter(loan => !loan.isReturned());
    const overdueLoans = activeLoans.filter(loan => loan.isOverdue());

    return {
      totalBooks: allBooks.length,
      availableBooks: allBooks.filter(book => book.isAvailable).length,
      borrowedBooks: allBooks.filter(book => !book.isAvailable).length,
      totalMembers: this.members.size,
      activeLoans: activeLoans.length,
      overdueLoans: overdueLoans.length,
      totalLoans: allLoans.length
    };
  }

  /**
   * 통계 출력
   */
  printStatistics(): void {
    const stats = this.getStatistics();
    console.log("\n📊 === 도서관 통계 ===");
    console.log(`전체 장서: ${stats.totalBooks}권`);
    console.log(`대출 가능: ${stats.availableBooks}권`);
    console.log(`대출 중: ${stats.borrowedBooks}권`);
    console.log(`전체 회원: ${stats.totalMembers}명`);
    console.log(`현재 대출: ${stats.activeLoans}건`);
    console.log(`연체: ${stats.overdueLoans}건`);
    console.log(`총 대출 이력: ${stats.totalLoans}건`);
  }
}
```

**index.ts (사용 예시)**
```typescript
import { LibraryManager } from "./managers/LibraryManager";
import { Book } from "./models/Book";
import { RegularMember, VIPMember } from "./models";

async function main() {
  console.log("=== 도서관 관리 시스템 ===\n");

  // 1. 도서관 생성
  const library = new LibraryManager();

  // 2. 책 추가
  console.log("📚 === 책 추가 ===");
  library.addBook(new Book("978-1", "TypeScript 프로그래밍", "홍길동", 2024, true, "non-fiction"));
  library.addBook(new Book("978-2", "클린 코드", "로버트 마틴", 2008, true, "non-fiction"));
  library.addBook(new Book("978-3", "해리 포터", "J.K. 롤링", 1997, true, "fiction"));

  // 3. 회원 등록
  console.log("\n👤 === 회원 등록 ===");
  const member1 = new RegularMember("M001", "홍길동");
  const member2 = new VIPMember("M002", "김철수");
  library.registerMember(member1);
  library.registerMember(member2);

  // 4. 대출
  console.log("\n📖 === 대출 ===");
  const loan1 = library.borrowBook("M001", "978-1");
  const loan2 = library.borrowBook("M001", "978-2");

  // 5. 통계
  library.printStatistics();

  // 6. 반납
  console.log("\n📥 === 반납 ===");
  if (loan1) {
    library.returnBook(loan1.id);
  }

  // 7. 최종 통계
  library.printStatistics();
}

main().catch(console.error);
```

---

## 🔧 실행 방법

**1. 프로젝트 설정**
```bash
mkdir library-system
cd library-system
npm init -y
npm install typescript @types/node --save-dev
npx tsc --init
```

**2. tsconfig.json 설정**
```json
{
  "compilerOptions": {
    "target": "ES2020",
    "module": "commonjs",
    "lib": ["ES2020"],
    "outDir": "./dist",
    "rootDir": "./src",
    "strict": true,
    "esModuleInterop": true,
    "skipLibCheck": true,
    "forceConsistentCasingInFileNames": true
  },
  "include": ["src/**/*"],
  "exclude": ["node_modules", "dist"]
}
```

**3. 실행**
```bash
npm run dev
```

---

## 📚 학습 리소스

### 추가 학습 자료

**객체지향 프로그래밍:**
- SOLID 원칙
- 디자인 패턴 (Observer, Strategy, Factory 등)
- 상속 vs 컴포지션

**TypeScript 고급:**
- 유틸리티 타입
- 조건부 타입
- 매핑 타입

**추천 도서:**
- "Head First Design Patterns"
- "Refactoring" by Martin Fowler

---

## 🎯 다음 프로젝트 예고

**Project 3: 날씨 대시보드 앱**

이제 마지막 프로젝트입니다:

- **난이도**: ⭐⭐⭐⭐☆
- **소요 시간**: 4-6시간
- **특징**:
  - 실시간 API 연동 중심
  - 데이터 시각화
  - 캐싱 전략
  - 에러 처리 고급
  - 완전 자율 진행

- **핵심 학습**:
  - 외부 API 마스터
  - 데이터 캐싱
  - 비동기 패턴 고급
  - 사용자 경험 최적화

준비되셨나요?

**다음 프로젝트에서 만나요! 🌤️**

