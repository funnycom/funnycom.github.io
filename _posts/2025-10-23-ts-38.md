---
title: "[TS] 고급 타입 기법 - 1"
date: 2025-10-23 14:00:00 +0900
categories: [웹 개발, 타입스크립트]
tags: [웹개발, 타입스크립트, TS, 고급타입]
pin: true
math: true
mermaid: true
---

## 🎯 학습 목표

* 조건부 타입으로 타입을 동적으로 생성할 수 있다
* infer 키워드로 타입을 추론하고 추출할 수 있다
* 매핑 타입으로 기존 타입을 변환할 수 있다
* 템플릿 리터럴 타입으로 문자열 타입을 조작할 수 있다
* 사용자 정의 타입 가드를 만들 수 있다
* is와 asserts 키워드를 활용할 수 있다
* 타입 단언을 올바르게 사용할 수 있다
* 인덱스 접근 타입으로 중첩된 타입을 추출할 수 있다

---

## 14.1 조건부 타입 (Conditional Types)

조건부 타입은 TypeScript의 가장 강력한 기능 중 하나입니다. **타입 레벨에서 if-else 문**을 사용하는 것과 같습니다.

### 기본 문법 (T extends U ? X : Y)

**문법:**
```typescript
T extends U ? X : Y
```

> 💡 **읽는 방법**: "T가 U에 할당 가능하면 X, 아니면 Y"

**기본 예제:**
```typescript
// 숫자면 "number", 아니면 "other"
type TypeName<T> = T extends number ? "number" : "other";

type A = TypeName<number>;      // "number"
type B = TypeName<string>;      // "other"
type C = TypeName<boolean>;     // "other"
type D = TypeName<42>;          // "number" (리터럴 타입도 number에 할당 가능)
```

**실전 예제 1: 배열 여부 확인**
```typescript
type IsArray<T> = T extends any[] ? "yes" : "no";

type Test1 = IsArray<string[]>;     // "yes"
type Test2 = IsArray<number[]>;     // "yes"
type Test3 = IsArray<string>;       // "no"
type Test4 = IsArray<[1, 2, 3]>;    // "yes" (튜플도 배열)

// 실전 사용: 배열이면 요소 타입, 아니면 그대로
type ArrayElement<T> = T extends any[] ? T[number] : T;

type Element1 = ArrayElement<string[]>;  // string
type Element2 = ArrayElement<number>;    // number
```

**실전 예제 2: null 체크**
```typescript
type NonNullable<T> = T extends null | undefined ? never : T;

type A = NonNullable<string | null>;           // string
type B = NonNullable<number | undefined>;      // number
type C = NonNullable<string | null | undefined>; // string

// 사용
function processValue<T>(value: T): NonNullable<T> {
  if (value === null || value === undefined) {
    throw new Error("Value cannot be null or undefined");
  }
  return value as NonNullable<T>;
}

const result1 = processValue("hello");  // string
const result2 = processValue(42);       // number
```

**실전 예제 3: 읽기 전용 여부**
```typescript
type IsReadonly<T, K extends keyof T> = 
  Readonly<Pick<T, K>> extends Pick<T, K> ? true : false;

interface User {
  readonly id: number;
  name: string;
}

type IdReadonly = IsReadonly<User, 'id'>;    // true
type NameReadonly = IsReadonly<User, 'name'>; // false
```

### 중첩 조건부 타입
```typescript
// 타입 분류하기
type TypeCategory<T> = 
  T extends string ? "string" :
  T extends number ? "number" :
  T extends boolean ? "boolean" :
  T extends undefined ? "undefined" :
  T extends Function ? "function" :
  "object";

type T1 = TypeCategory<"hello">;      // "string"
type T2 = TypeCategory<42>;           // "number"
type T3 = TypeCategory<true>;         // "boolean"
type T4 = TypeCategory<() => void>;   // "function"
type T5 = TypeCategory<{ a: 1 }>;     // "object"

// 실전 사용: 타입별 처리
function processValue<T>(value: T): string {
  type Category = TypeCategory<T>;
  
  if (typeof value === "string") {
    return `String: ${value}`;
  }
  if (typeof value === "number") {
    return `Number: ${value}`;
  }
  if (typeof value === "boolean") {
    return `Boolean: ${value}`;
  }
  return `Other: ${value}`;
}
```

### 분산 조건부 타입 (Distributive Conditional Types)

유니온 타입에 조건부 타입을 적용하면 **각 타입에 개별적으로** 적용됩니다.
```typescript
type ToArray<T> = T extends any ? T[] : never;

// 유니온 타입에 적용하면 분산됨
type StringOrNumber = string | number;
type Result = ToArray<StringOrNumber>;
// 결과: string[] | number[]
// (ToArray<string> | ToArray<number>)

// 단계별 설명:
// 1. ToArray<string | number>
// 2. ToArray<string> | ToArray<number>  (분산)
// 3. string[] | number[]

// 실전 예제: 배열 래핑
type Arrayify<T> = T extends any ? T[] : never;

type Test = Arrayify<string | number | boolean>;
// string[] | number[] | boolean[]

const arr1: Test = ["hello"];        // ✅ OK
const arr2: Test = [42];             // ✅ OK
const arr3: Test = [true];           // ✅ OK
const arr4: Test = ["hello", 42];    // ❌ 에러!
```

**분산 방지하기:**
```typescript
// 대괄호로 감싸면 분산되지 않음
type ToArrayNonDistributive<T> = [T] extends [any] ? T[] : never;

type Result2 = ToArrayNonDistributive<string | number>;
// 결과: (string | number)[]  (분산 안 됨)

const arr: Result2 = ["hello", 42]; // ✅ OK (혼합 가능)
```

### 타입 추론 (infer)

`infer` 키워드는 조건부 타입 내에서 **타입을 추론하고 추출**합니다.

**기본 문법:**
```typescript
T extends infer U ? U : never
```

> 💡 **infer U**: "여기서 타입을 추론해서 U에 저장해줘"

**예제 1: 반환 타입 추출**
```typescript
// ReturnType 구현
type MyReturnType<T> = T extends (...args: any[]) => infer R ? R : never;

function getUser() {
  return { id: 1, name: "홍길동" };
}

type User = MyReturnType<typeof getUser>;
// 결과: { id: number; name: string; }

// 사용
const user: User = { id: 2, name: "김철수" }; // ✅ OK
```

**예제 2: 매개변수 타입 추출**
```typescript
// Parameters 구현
type MyParameters<T> = T extends (...args: infer P) => any ? P : never;

function createUser(name: string, age: number, email: string) {
  return { name, age, email };
}

type Params = MyParameters<typeof createUser>;
// 결과: [name: string, age: number, email: string]

// 사용
const params: Params = ["홍길동", 30, "hong@example.com"];
const user = createUser(...params); // ✅ OK
```

**예제 3: 배열 요소 타입 추출**
```typescript
type ArrayElementType<T> = T extends (infer E)[] ? E : never;

type StringArray = string[];
type Element1 = ArrayElementType<StringArray>;  // string

type NumberArray = number[];
type Element2 = ArrayElementType<NumberArray>;  // number

type MixedArray = (string | number)[];
type Element3 = ArrayElementType<MixedArray>;   // string | number

// 실전 사용
function getFirstElement<T extends any[]>(arr: T): ArrayElementType<T> | undefined {
  return arr[0];
}

const first1 = getFirstElement(["a", "b", "c"]);  // string | undefined
const first2 = getFirstElement([1, 2, 3]);        // number | undefined
```

**예제 4: Promise 언래핑**
```typescript
type Awaited<T> = T extends Promise<infer U> ? U : T;

type A = Awaited<Promise<string>>;      // string
type B = Awaited<Promise<number>>;      // number
type C = Awaited<string>;               // string (Promise 아님)

// 중첩 Promise 처리
type DeepAwaited<T> = 
  T extends Promise<infer U> 
    ? DeepAwaited<U> 
    : T;

type Nested = Promise<Promise<Promise<string>>>;
type Unwrapped = DeepAwaited<Nested>;  // string

// 실전 사용
async function fetchData(): Promise<{ id: number; name: string }> {
  return { id: 1, name: "데이터" };
}

type Data = Awaited<ReturnType<typeof fetchData>>;
// { id: number; name: string }
```

**예제 5: 객체 속성 타입 추출**
```typescript
type PropertyType<T, K extends string> = 
  T extends { [key in K]: infer V } ? V : never;

interface User {
  id: number;
  name: string;
  email: string;
}

type NameType = PropertyType<User, 'name'>;    // string
type IdType = PropertyType<User, 'id'>;        // number

// 실전 사용: 깊은 속성 접근
type DeepPropertyType<T, Path extends string> =
  Path extends `${infer K}.${infer Rest}`
    ? K extends keyof T
      ? DeepPropertyType<T[K], Rest>
      : never
    : Path extends keyof T
      ? T[Path]
      : never;

interface Company {
  name: string;
  ceo: {
    name: string;
    age: number;
  };
}

type CeoName = DeepPropertyType<Company, 'ceo.name'>;  // string
type CeoAge = DeepPropertyType<Company, 'ceo.age'>;    // number
```

### 실전 예제: 타입 안전한 이벤트 시스템
```typescript
// 이벤트 맵 정의
interface EventMap {
  'user:login': { userId: number; timestamp: Date };
  'user:logout': { userId: number };
  'post:created': { postId: number; title: string };
  'post:deleted': { postId: number };
}

// 이벤트 이름 추출
type EventName = keyof EventMap;

// 특정 이벤트의 데이터 타입 추출
type EventData<T extends EventName> = EventMap[T];

// 이벤트 핸들러 타입
type EventHandler<T extends EventName> = (data: EventData<T>) => void;

// 타입 안전한 이벤트 이미터
class TypedEventEmitter {
  private handlers: {
    [K in EventName]?: EventHandler<K>[];
  } = {};

  on<T extends EventName>(event: T, handler: EventHandler<T>) {
    if (!this.handlers[event]) {
      this.handlers[event] = [];
    }
    (this.handlers[event] as EventHandler<T>[]).push(handler);
  }

  emit<T extends EventName>(event: T, data: EventData<T>) {
    const eventHandlers = this.handlers[event];
    if (eventHandlers) {
      eventHandlers.forEach(handler => handler(data));
    }
  }
}

// 사용
const emitter = new TypedEventEmitter();

// ✅ 타입 안전!
emitter.on('user:login', (data) => {
  console.log(`User ${data.userId} logged in at ${data.timestamp}`);
  // data는 { userId: number; timestamp: Date } 타입
});

emitter.emit('user:login', {
  userId: 1,
  timestamp: new Date()
}); // ✅ OK

emitter.emit('user:login', {
  userId: 1
  // ❌ 에러! timestamp가 없음
});

emitter.on('post:created', (data) => {
  console.log(`Post ${data.postId}: ${data.title}`);
  // data는 { postId: number; title: string } 타입
});
```

### 조건부 타입 활용 패턴

**패턴 1: 함수인지 확인**
```typescript
type IsFunction<T> = T extends (...args: any[]) => any ? true : false;

type T1 = IsFunction<() => void>;        // true
type T2 = IsFunction<(x: number) => string>; // true
type T3 = IsFunction<string>;            // false
type T4 = IsFunction<number>;            // false
```

**패턴 2: 선택적 속성만 추출**
```typescript
type OptionalKeys<T> = {
  [K in keyof T]-?: {} extends Pick<T, K> ? K : never;
}[keyof T];

interface User {
  id: number;
  name: string;
  email?: string;
  phone?: string;
}

type UserOptionalKeys = OptionalKeys<User>;
// 'email' | 'phone'
```

**패턴 3: Flatten (중첩 배열 평탄화)**
```typescript
type Flatten<T> = T extends (infer U)[] ? Flatten<U> : T;

type NestedArray = string[][][];
type Flattened = Flatten<NestedArray>;  // string

type MixedNested = (number | string[])[];
type MixedFlat = Flatten<MixedNested>;  // number | string
```

---

## 14.2 매핑 타입 (Mapped Types)

매핑 타입은 **기존 타입의 각 속성을 순회하며 새로운 타입을 생성**합니다.

### 기본 매핑 타입

**문법:**
```typescript
type MappedType = {
  [P in Keys]: Type
}
```

> 💡 **읽는 방법**: "Keys의 각 P에 대해, Type을 가진 속성 생성"

**기본 예제:**
```typescript
type Keys = 'name' | 'age' | 'email';

// 모든 키를 string 타입으로
type StringRecord = {
  [K in Keys]: string;
};

/*
결과:
{
  name: string;
  age: string;
  email: string;
}
*/

const user: StringRecord = {
  name: "홍길동",
  age: "30",        // 숫자가 아닌 문자열!
  email: "hong@example.com"
};
```

### keyof를 활용한 매핑

**예제 1: 모든 속성을 boolean으로**
```typescript
interface User {
  id: number;
  name: string;
  email: string;
}

// 모든 속성을 boolean으로 변환
type BooleanUser = {
  [K in keyof User]: boolean;
};

/*
결과:
{
  id: boolean;
  name: boolean;
  email: boolean;
}
*/

// 사용: 플래그 객체
const flags: BooleanUser = {
  id: true,
  name: false,
  email: true
};
```

**예제 2: 모든 속성을 배열로**
```typescript
type Arrayify<T> = {
  [K in keyof T]: T[K][];
};

interface Product {
  id: number;
  name: string;
  price: number;
}

type ProductArrays = Arrayify<Product>;

/*
결과:
{
  id: number[];
  name: string[];
  price: number[];
}
*/

const products: ProductArrays = {
  id: [1, 2, 3],
  name: ["상품A", "상품B"],
  price: [1000, 2000]
};
```

**예제 3: 모든 속성을 nullable로**
```typescript
type Nullable<T> = {
  [K in keyof T]: T[K] | null;
};

interface User {
  id: number;
  name: string;
  email: string;
}

type NullableUser = Nullable<User>;

/*
결과:
{
  id: number | null;
  name: string | null;
  email: string | null;
}
*/

const user: NullableUser = {
  id: 1,
  name: "홍길동",
  email: null  // ✅ OK
};
```

### 매핑 타입 수정자

**+/- 수정자:**
```typescript
// +?: 선택적 속성 추가
type Partial<T> = {
  [K in keyof T]+?: T[K];
};

// -?: 선택적 속성 제거 (필수로 만들기)
type Required<T> = {
  [K in keyof T]-?: T[K];
};

// +readonly: 읽기 전용 추가
type Readonly<T> = {
  +readonly [K in keyof T]: T[K];
};

// -readonly: 읽기 전용 제거
type Mutable<T> = {
  -readonly [K in keyof T]: T[K];
};
```

**실전 예제:**
```typescript
interface User {
  readonly id: number;
  name: string;
  email?: string;
}

// 모든 readonly 제거, 모든 ? 제거
type MutableRequired<T> = {
  -readonly [K in keyof T]-?: T[K];
};

type EditableUser = MutableRequired<User>;

/*
결과:
{
  id: number;        // readonly 제거됨
  name: string;
  email: string;     // ? 제거됨
}
*/

const user: EditableUser = {
  id: 1,
  name: "홍길동",
  email: "hong@example.com"
};

user.id = 2;  // ✅ OK (이제 변경 가능)
```

### as를 사용한 키 재매핑

**문법:**
```typescript
type MappedType = {
  [K in Keys as NewKey]: Type
}
```

**예제 1: 접두사 추가**
```typescript
type Getters<T> = {
  [K in keyof T as `get${Capitalize<string & K>}`]: () => T[K];
};

interface User {
  name: string;
  age: number;
}

type UserGetters = Getters<User>;

/*
결과:
{
  getName: () => string;
  getAge: () => number;
}
*/

const userGetters: UserGetters = {
  getName: () => "홍길동",
  getAge: () => 30
};

console.log(userGetters.getName());  // "홍길동"
console.log(userGetters.getAge());   // 30
```

**예제 2: 특정 타입 필터링**
```typescript
// 함수가 아닌 속성만 선택
type NonFunctionProperties<T> = {
  [K in keyof T as T[K] extends Function ? never : K]: T[K];
};

interface User {
  id: number;
  name: string;
  getName: () => string;
  getAge: () => number;
}

type UserData = NonFunctionProperties<User>;

/*
결과:
{
  id: number;
  name: string;
}
*/
```

**예제 3: 키 변환**
```typescript
// snake_case를 camelCase로
type CamelCase<S extends string> = 
  S extends `${infer P1}_${infer P2}${infer P3}`
    ? `${P1}${Uppercase<P2>}${CamelCase<P3>}`
    : S;

type CamelCaseKeys<T> = {
  [K in keyof T as CamelCase<string & K>]: T[K];
};

interface ApiResponse {
  user_id: number;
  user_name: string;
  created_at: string;
}

type CamelResponse = CamelCaseKeys<ApiResponse>;

/*
결과:
{
  userId: number;
  userName: string;
  createdAt: string;
}
*/
```

### 커스텀 유틸리티 타입 만들기

**예제 1: PartialBy - 특정 키만 선택적으로**
```typescript
type PartialBy<T, K extends keyof T> = Omit<T, K> & Partial<Pick<T, K>>;

interface User {
  id: number;
  name: string;
  email: string;
  age: number;
}

// email과 age만 선택적으로
type UserPartial = PartialBy<User, 'email' | 'age'>;

/*
결과:
{
  id: number;
  name: string;
  email?: string;
  age?: number;
}
*/
```

**예제 2: RequiredBy - 특정 키만 필수로**
```typescript
type RequiredBy<T, K extends keyof T> = Omit<T, K> & Required<Pick<T, K>>;

interface Config {
  host?: string;
  port?: number;
  username?: string;
  password?: string;
}

// host와 port만 필수로
type RequiredConfig = RequiredBy<Config, 'host' | 'port'>;

/*
결과:
{
  host: string;
  port: number;
  username?: string;
  password?: string;
}
*/
```

**예제 3: ReadonlyBy - 특정 키만 읽기 전용으로**
```typescript
type ReadonlyBy<T, K extends keyof T> = Omit<T, K> & Readonly<Pick<T, K>>;

interface User {
  id: number;
  name: string;
  email: string;
}

// id만 읽기 전용으로
type UserWithReadonlyId = ReadonlyBy<User, 'id'>;

/*
결과:
{
  readonly id: number;
  name: string;
  email: string;
}
*/

const user: UserWithReadonlyId = {
  id: 1,
  name: "홍길동",
  email: "hong@example.com"
};

user.name = "김철수";  // ✅ OK
user.id = 2;  // ❌ 에러! readonly
```

**예제 4: PickByType - 특정 타입의 속성만 선택**
```typescript
type PickByType<T, ValueType> = {
  [K in keyof T as T[K] extends ValueType ? K : never]: T[K];
};

interface User {
  id: number;
  name: string;
  email: string;
  age: number;
  isActive: boolean;
}

// 문자열 타입만
type StringFields = PickByType<User, string>;

/*
결과:
{
  name: string;
  email: string;
}
*/

// 숫자 타입만
type NumberFields = PickByType<User, number>;

/*
결과:
{
  id: number;
  age: number;
}
*/
```

**예제 5: Transform - 속성 타입 변환**
```typescript
type Transform<T, From, To> = {
  [K in keyof T]: T[K] extends From ? To : T[K];
};

interface User {
  id: number;
  name: string;
  age: number;
  email: string;
}

// 모든 number를 string으로
type StringifiedUser = Transform<User, number, string>;

/*
결과:
{
  id: string;
  name: string;
  age: string;
  email: string;
}
*/
```

### 실전 예제: API 응답 타입 변환
```typescript
// API 응답 (snake_case)
interface ApiUser {
  user_id: number;
  user_name: string;
  email_address: string;
  created_at: string;
  updated_at: string;
}

// 1. snake_case를 camelCase로
type ToCamelCase<S extends string> = 
  S extends `${infer P1}_${infer P2}${infer P3}`
    ? `${P1}${Uppercase<P2>}${ToCamelCase<P3>}`
    : S;

type CamelCaseKeys<T> = {
  [K in keyof T as ToCamelCase<string & K>]: T[K];
};

// 2. 날짜 문자열을 Date로
type ParseDates<T> = {
  [K in keyof T]: K extends `${string}At` 
    ? Date 
    : T[K];
};

// 조합
type AppUser = ParseDates<CamelCaseKeys<ApiUser>>;

/*
결과:
{
  userId: number;
  userName: string;
  emailAddress: string;
  createdAt: Date;
  updatedAt: Date;
}
*/

// 변환 함수
function transformApiUser(apiUser: ApiUser): AppUser {
  return {
    userId: apiUser.user_id,
    userName: apiUser.user_name,
    emailAddress: apiUser.email_address,
    createdAt: new Date(apiUser.created_at),
    updatedAt: new Date(apiUser.updated_at)
  };
}
```

